...
export function PortfolioSummary({ aggregates }: PortfolioSummaryProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <MetricCard
          ...
          value={formatCurrency(aggregates.totalProjectedSalePrice)}
          icon={DollarSign}
          size="sm"
        />
        ...
        <MetricCard
          label="Total Basis"
          value={formatCurrency(aggregates.totalAcquisitionBasis)}
          icon={Building2}
          size="sm"
        />
        ...
        <MetricCard
          label="Selling Costs"
          value={formatCurrency(aggregates.totalSellingCosts)}
          subValue={formatPercent(
            aggregates.totalProjectedSalePrice > 0
              ? aggregates.totalSellingCosts / aggregates.totalProjectedSalePrice
              : 0
          )}
          icon={Receipt}
          size="sm"
        ...
        <MetricCard
          label="Net Proceeds"
          value={formatCurrency(aggregates.totalNetSaleProceeds)}
          icon={PiggyBank}
          size="sm"
        />
        ...
        <MetricCard
          ...
          value={`${isGain ? '+' : ''}${formatCurrency(aggregates.totalGainLossVsBasis)}`}
        ...
        <MetricCard
          label="Portfolio ROI"
          value={formatPercent(aggregates.weightedAverageROI)}
          subValue={
            aggregates.portfolioIRR !== undefined
              ? `${formatPercent(aggregates.portfolioIRR)} IRR`
              : `${aggregates.averageHoldPeriodYears.toFixed(1)} yr avg hold`
          }
          trend={aggregates.weightedAverageROI >= 0 ? 'up' : 'down'}
          icon={Percent}
          size="sm"
        />
      </div>
